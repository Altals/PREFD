...
为了推导（19.5）数值解的有限差分算法，我们遵循第5.1节中的相同路径来推导微分的前向差分算法。我们首先将势的两个泰勒展开式添加到（x，y）的右边和左边，以及上面和下面（x，y）的两个展开式：
...
我们假定x和y网格具有相等的间距Δx=Δy=Δ，从而获得方程的一个简单形式



读者会注意到，这个方程显示了空间中五个点的解之间的关系。当对格上的Nx x值和Ny y值评估U（x，y）时，我们得到了一组Nx×Ny联立线性代数方程，供U[i，j]求解。一种方法是将这些方程明确地作为一个（大）矩阵问题来求解。这很有吸引力，因为这是一个直接的解决方案，但它需要大量的内存和核算。我们使用的方法来自U（x，y）的（19.28）代数解：
...
在这一点上，重要的是要记住，我们的算法是由在直角坐标系中对拉普拉斯算子Ş2进行外压而产生的。虽然这并不限制我们解决圆对称问题，但在某些几何体中，最好开发一种基于在柱坐标或球面坐标中表达拉普拉斯算子的算法，以使网格更适合几何体